<template>
  ...
  <div class="projects" v-if="user.id !== undefined">
    ...
    >
      ID: {{ user.id }}
      <br />
      Username [Display Name]: {{ user.username }} [{{ user.displayName }}]
      <br />
      ...
      :buttons="[
        {
          ...
          action: handleBan,
          text: user.bannedAt ? 'Release them!' : 'Knock \'em into next year!'
        }
      ]"
      :cancelable="true"
    >
      ...
      :buttons="[
        {
          ...
          title: 'Send them off, they won\'t be back (well, not this account)'
        }
      ]"
      :cancelable="true"
    >
      By deleting their account, they will NEVER be able to access this account
      again, and ALL of their files will be deleted. If you're absolutely sure
      you want to do this, press confirm.
    </Modal>
    <Project
          ...
          res => {
            switch (res.status) {
              case 200:
                res.json().then(json => {
                  this.$parent.$parent.temporaryToast(
                    `Success! ${this.user.username}'s file link is now ${
                      json.subdomain
                    }${json.subdomain !== '' ? '.' : ''}${json.domain}`
                  );

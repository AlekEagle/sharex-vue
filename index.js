    ...
    user
      ...
      .then(
        u => {
          if (u !== null) {
            if (u.bannedAt === null) {
              bcrypt.compare(password, u.password).then(
                match => {
                  if (match) {
                    actions
                      .create({
        ...
        .then(ins => {
          if (ins !== null) {
            ...
            res
              .status(200)
              .send(
                ins.fileContent.replace(/(\{\{apiToken\}\})/g, `${u.apiToken}`)
              );
          } else {
            res.status(404).json({
              error: 'Not Found'
  ...
  authenticate(req).then(
    u => {
      ...
      res.set(
        ...
        'https://docs.alekeagle.me/api/user.html#upload'
      );
      if (!req.file) {
        if (!req.body.file) {
          res.status(400).json({ error: 'Bad Request', missing: ['file'] });
